...
namespace FastNet
{
    ...
    /// </summary>
    public partial class KCP_BE : KCP
    {
        ...
        public static byte ikcp_decode8u(byte[] p, int offset,ref byte c)
        {
            c = p[0 + offset];
            return 1;
        }
        ...
        {
            UInt16 result = 0;
            result |= (UInt16)((p[offset + 0] & 0xFF) << 8);
            result |= (UInt16)(p[offset + 1] & 0xFF);
            c = result;
            return 2;
        }
        ...
        public static int ikcp_decode32u(byte[] p, int offset, ref UInt32 l)
        {
            l = (UInt32)((p[offset + 0] & 0xFFL) << 24
                    | (p[offset + 1] & 0xFFL) << 16
                    | (p[offset + 2] & 0xFFL) << 8
                    | p[offset + 3] & 0xFFL);
            return 4;
        ...
        ConcurrentQueue<Segment> snd_queue = new ConcurrentQueue<Segment>();
        ConcurrentQueue<Segment> rcv_queue = new ConcurrentQueue<Segment>();
        ...
        public KCP_BE(UInt32 conv_, Action<byte[], int> output_, byte[] outputBuffer = null)
        {
            conv = conv_;
        ...
        public override int Recv(byte[] buffer)
        {
        ...
        public override int Input(byte[] data)
        {
            ...
            while (true)
            {
                ...
                //Console.WriteLine($"{offset}:cmd:{cmd},{frg},{wnd},{ts},{sn},{una},length:{length}");
                ...
                else if (IKCP_CMD_PUSH == cmd)
                {
                    if (_itimediff(sn, rcv_nxt + rcv_wnd) < 0)
                    {
                        ...
                        if (_itimediff(sn, rcv_nxt) >= 0)
                        {
                            ...
                            if (length > 0) Buffer.BlockCopy(data, offset, seg.data, 0, (int)length);
        ...
        public override void Update(long current_L_)
        {
        ...
        public override long Check(long current_L_)
        {
            UInt32 current_ = (UInt32)current_L_;
        ...
        public override int SetMtu(Int32 mtu_)
        {
        ...
        public override int Interval(Int32 interval_)
        {
            if (interval_ > 5000)
            {
                interval_ = 5000;
        ...
        public override int NoDelay(int nodelay_, int interval_, int resend_, int nc_)
        {
        ...
        public override int WndSize(int sndwnd, int rcvwnd)
        {
            if (sndwnd > 0)
        ...
        {
            snd_queue = new ConcurrentQueue<Segment>();
            rcv_queue = new ConcurrentQueue<Segment>();
            snd_buf = new List<Segment>();
            rcv_buf = new List<Segment>();
